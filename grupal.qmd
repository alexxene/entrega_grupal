...
Hemos descubierto que las diferencias principales están en en CIU y UP en 2015.
Resumen le da 630.000 votos a CIU, pero resumen2 solo 65.000, según la web del ministerio del interior obtuvieron 567.000 (ver fichero Elecciones.xls en el repositorio de Github).
...
```{r}
resultados <- resumen_2 |> 
  group_by(fecha, siglas) |>                      
  summarise(Votos_Totales = sum(votos), .groups = "drop") |> 
  group_by(fecha) |> 
  mutate(Porcentaje = Votos_Totales / sum(Votos_Totales) * 100) |> 
  select(fecha, siglas, Porcentaje) |> 
  pivot_wider(names_from = siglas, values_from = Porcentaje)
casas_encuestadoras <- surveys |> 
  select(-c(id_pollster,media,field_date_from,field_date_to,exit_poll,turnout)) |> 
  group_by(date_elec,pollster) |> 
  summarise(across(everything(), mean, na.rm = FALSE))
#el valor que aparece en cada pollster para cada eleccion en los partidos es la media ya que alexandra y yo hemos pensado que para unirlas numerosas realizadas para cada eleccion por cada casa hariamos la media y asi obtendríamos un valor único para cada partido con el que podemos comparar
