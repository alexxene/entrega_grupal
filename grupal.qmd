...
surveys <- surveys|> select(-type_survey) |>
  ...
  filter((field_date_to-field_date_from)>1) |> #he puesto que sean mayores que uno ya que en el punto 11 pone que tengan mas de un día y si ponemos 0 incluye los que tienen 1 dia de trabajo de campo
...
# Para saber qué partidos están relacionados con "Unidas Podemos" miramos en la base de datos abbrev.
abbrev |>
  mutate("Contar_IU" = str_detect(siglas, "IU|Podem|Ezker Batua")) |> 
  filter(Contar_IU == TRUE)
...
resumen2 <- resumen |> 
  mutate(siglas = case_when(
    # Agrupación de partidos bajo las reglas predefinidas
    str_detect(siglas, regex("IU|EU|EB-B|ENTESA|PODEMOS|EZKERRA|UP-UPeC|I\\.U\\.", ignore_case = TRUE)) ~ 'UP',
    ...
    str_detect(siglas, regex("aralar|^EA$|AMAIUR|EH", ignore_case = TRUE)) ~ 'EH-BILDU',
    str_detect(siglas, regex("CiU", ignore_case = TRUE)) ~ 'CIU',
    str_detect(siglas, regex("ERC", ignore_case = TRUE)) ~ 'ERC',
    str_detect(siglas, regex("BNG", ignore_case = TRUE)) ~ 'BNG',
    # Verificación adicional en 'partidos'
    str_detect(partidos, regex("PARTIT POPULAR|\\(PP\\)", ignore_case = TRUE)) ~ 'PP',
    ...
    str_detect(partidos, regex("IZQUIERDA UNIDA", ignore_case = TRUE)) ~ 'UP',
    # Asignar 'OTRO' si no cumple ninguna regla
    TRUE ~ 'OTRO'
  ))
#con este codigo conseguimos que se agrupen los partidos pero si veis los resultados hay algunos que no lo han hecho y eso es porque en la tabla, tienen el valor de siglas como NA si sabeis como podemos arreglarlo hacedlo porque ya no se que hacer
prueba <- resumen2 |> filter(siglas == 'OTRO')
