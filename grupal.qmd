...
#Mantenemos participación 1 y 2 
election_data <- 
  election_data |> 
...
resumen <- 
  ...
  TRUE ~ "OTROS"
)) |> pivot_longer(cols = c("participacion_1", "participacion_2"), names_to = "sesion", values_to = "votos_sesion") |> relocate(sesion, .before = votos_blancos) |> relocate(votos_sesion, .before = votos_blancos)
  ...
  resumen_2 |> 
   mutate(siglas = case_when(
    ...
    # Partidos que no aparecen en abbrev que tenemos que incluir'
     ...
     TRUE ~ 'OTROS')) |> pivot_longer(cols = c("participacion_1", "participacion_2"), names_to = "sesion", values_to = "votos_sesion") |> relocate(sesion, .before = votos_blancos) |> relocate(votos_sesion, .before = votos_blancos)
...
grafico_resultados <- ggplot(resultados, aes(x = siglas, y = Porcentaje_real, fill = siglas)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
...
#  theme_minimal()+
  scale_fill_manual(values = colores) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1), # Rotar etiquetas en eje X
    legend.position = "none" # Ocultar leyenda, ya que los colores se explican con las etiquetas
  ) +
    facet_wrap(~fecha) # Un gráfico por año

grafico_casas <- ggplot(casas_encuestadoras, aes(x = partidos, y = porcentaje_estimado, color = partidos, label=pollster)) +
  geom_point(shape = 16, size = 0.5, position = position_jitter(width = 0.1, height = 0)) + # Puntos de estimación de votos
  labs(
    title = "Estimación de votos por partido",
    x = "Partido",
    y = "Estimación de votos (%)"
  ) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1), # Rotar etiquetas en eje X
    legend.position = "none" # Ocultar leyenda
  ) +
  facet_wrap(~date_elec) + # Un gráfico por año
  scale_color_manual(values = colores) # Asignar colores personalizados

library(plotly)

graf_int_resultados <- ggplotly(grafico_resultados)
graf_int_casas <- ggplotly(grafico_casas)
